...
## 在地图任意位置生成一个铁矿瓦块
## 返回：生成的铁矿瓦块位置，如果失败返回 null
func generate_iron_ore_random() -> Vector2i:
	...
	# 创建铁矿瓦块数据
	var iron_ore_tile = IronOreTile.new()
	
	# 存储在Tile中
	_tile_manager.SetResourceTile(random_pos, iron_ore_tile)
...
## count: 要生成的铁矿数量
## 返回：生成的铁矿位置数组
func generate_iron_ores(count: int = 15) -> Array[Vector2i]:
	var iron_ore_positions: Array[Vector2i] = []
	var attempts = 0
	var max_attempts = count * 50  # 最多尝试次数
	
	while iron_ore_positions.size() < count and attempts < max_attempts:
		var pos = generate_iron_ore_random()
		if pos.x >= 0 and pos.y >= 0:  # 有效位置
			iron_ore_positions.append(pos)
		attempts += 1
	
	return iron_ore_positions
